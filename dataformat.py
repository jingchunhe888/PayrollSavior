...
#             next_value = next_row[col]
#
#             if pd.notna(current_value) and pd.notna(next_value):
#                 # Both rows have values in this column; skip merging
#                 skip_merge = True
#                 all_rows.append(current_row.tolist())
#                 break
#             elif pd.notna(current_value) and pd.isna(next_value):
#                 # Current row has a value and next row has NaN
#                 merge = True
#                 break
#             elif pd.isna(current_value) and pd.notna(next_value):
#                 # Current row has NaN and next row has a value
#                 merge = True
#                 break
#             elif pd.isna(current_value) and pd.isna(next_value):
#                 # Both are NaN; we don't need to merge as no useful data is available
#                 merge = True
#                 break
#
...
#             # Merge the rows
#             for col in df_copy.columns:
#                 if pd.isna(current_row[col]) and not pd.isna(next_row[col]):
